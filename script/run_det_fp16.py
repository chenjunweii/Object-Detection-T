...
params['mean'] = nd.array(np.array([r, g, b]).astype(np.float16).reshape([1, 3, 1, 1]))
...
with compiler.build_config(opt_level = 3):
    graph, lib, params = compiler.build(net, target, {"data": shapes, "mean" : (1, 3, 1, 1)}, dtype = 'float16')
...
inputs = np.zeros(shapes).astype(np.float16)
