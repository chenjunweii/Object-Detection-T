#ifndef FLT_DARKNET_YOLO_HH
#define FLT_DARKNET_YOLO_HH
...
/*
int get_yolo_detections(vector <float> l, int w, int h, int netw, int neth, float thresh, int * map, int relative, detection * dets){
...
detection * get_detections(vector <vector <float>> layer_out, vector <int> lw, vector <int> lh, int w, int h, int * nboxes){
	...
	cout << "Before Make YOLO Layer " << endl;

    for(int i=0; i < layer_out.size(); ++i){

        layer l_params = make_yolo_layer(1, lw[i], lh[i], numBBoxes, numAnchors, classes);
    ...
    //get network boxes
    detection * dets = get_network_boxes(layers_params, w, h, thresh, hier_thresh, 0, relative, nboxes);
