...
class tuner(object):
    ...
    def tune(self):
        ...
        dtypes['reshape1_attr'] = 'int32'

        ff = ['reshape5_mask', 'reshape5_bias', 'reshape5_attr', 'reshape3_mask', 'reshape3_bias', 'reshape3_attr', 'reshape1_mask', 'reshape1_bias']
        ...
        self.tasks = autotvm.task.extract_from_graph(self.net, target = self.target, target_host = self.target_host,
                                                shape = {'data': self.input_shape}, dtype = dtypes,
        ...
        self.tasks = autotvm.task.extract_from_program(self.net, target=self.target,
                                            params=self.params, ops=(tvm.relay.op.nn.conv2d, tvm.relay.op.nn.dense)) 
